...
abstract class AdyenDropInService : Service(), CoroutineScope, AdyenDropInServiceInterface {
    ...
    private val resultLiveData: MutableLiveData<AdyenDropInServiceResult> = MutableLiveData()
     ...
     */
    protected fun sendResult(result: AdyenDropInServiceResult) {
     ...
     */
    open fun makePaymentsCall(paymentComponentJson: JSONObject): AdyenDropInServiceResult {
     ...
     * @return The result of the network call
     */
    open fun makeDetailsCall(actionComponentJson: JSONObject): AdyenDropInServiceResult {
        throw NotImplementedError("Neither makeDetailsCall nor onDetailsCallRequested is implemented")
    }

    override fun observeResult(owner: LifecycleOwner, observer: Observer<AdyenDropInServiceResult>) {
        this.resultLiveData.observe(owner, observer)
    }

    internal inner class DropInBinder : Binder() {
        fun getService(): AdyenDropInServiceInterface = this@AdyenDropInService
    }
...
internal interface AdyenDropInServiceInterface {
    fun observeResult(owner: LifecycleOwner, observer: Observer<AdyenDropInServiceResult>)
